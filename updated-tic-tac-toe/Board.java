...
public class Board {
	...
	public void computerMove(User user) {
		Random random = new Random();
		int randomNumber = random.nextInt(totalPossibleMoves);
		inputBoard[row[randomNumber]][column[randomNumber]] = user.COMPUTER_CHARACTER;
	}
	 ...
	 *             //when error occur in input or output
	 */
	void nextMove(User user) throws NumberFormatException, IOException {
		...
		inputBoard[inputRow][inputColumn] = user.USER_CHARACTER;
	...
	public char whoWinWhoLoss() {

		// check all the rows there are some match is present or not
		for (int i = 1; i < 4; i++) {
			if (inputBoard[i][2] == inputBoard[i][1]
					&& inputBoard[i][2] == inputBoard[i][3]
					&& inputBoard[i][2] != 0) {
				this.gameOn = false;
				return inputBoard[i][2];
			}
		}
		// now check the all column
		for (int i = 1; i < 4; i++) {
			if (inputBoard[2][i] == inputBoard[1][i]
					&& inputBoard[2][i] == inputBoard[3][i]
					&& inputBoard[2][i] != 0) {
				this.gameOn = false;
				return inputBoard[2][i];
			}
		}
		// now check for left top to right down
		if (inputBoard[2][2] == inputBoard[1][1]
				&& inputBoard[2][2] == inputBoard[3][3]
				&& inputBoard[2][2] != 0) {
			this.gameOn = false;
			return inputBoard[2][2];
		}
		// now check from left down to right top
		if (inputBoard[2][2] == inputBoard[3][1]
				&& inputBoard[2][2] == inputBoard[1][3]
				&& inputBoard[2][2] != 0) {
			this.gameOn = false;
			return inputBoard[2][2];
		}
	...
	public void playerMoves(User user) throws NumberFormatException,
			IOException {
		...
		// assign correct move to the board
		inputBoard[inputRow][inputColumn] = user.PLAYER1;
		...
		if (gameOn == true) {
			...
			inputBoard[inputRow][inputColumn] = user.PLAYER2;
		}
		whoWinWhoLoss();
		// if any user win then show board
		if (gameOn == false) {
